import pytest
import numpy as np
import maat_tracking.preset_model.preset_control
...
def test_good_cos_similarity(a, b, expected):
    assert pytest.approx(
        maat_tracking.preset_model.preset_control.cos_similarity(a, b)) == expected


@pytest.mark.parametrize("a, b", generate_bad_pairs_of_vectors())
def test_bad_weather_cos_similarity(a, b):
    with pytest.raises(TypeError) as excinfo:
        maat_tracking.preset_model.preset_control.cos_similarity(a, b)
...
def test_good_presets(current, presets, expected):
    assert maat_tracking.preset_model. \
