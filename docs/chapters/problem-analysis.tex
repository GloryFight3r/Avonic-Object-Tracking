...
First, we look at the user story in \ref{sec:problem-statement}.
Secondly, we consider three cases of other companies that have solved
a similar problem and explain how they relate to our own project in \ref{sec:available-solutions}
Then we describe the stakeholders in \ref{sec:stakeholders} and assess potential bottlenecks
we could have run into inside \ref{sec:possible-pitfalls}.
Finally, in \ref{sec:ethical-implications} we take a look at the ethical implications of our project.
...
a camera system that follows the speaker as they move around in a room. The way they want to do
...
To solve this problem, Avonic wants to try to use a microphone array in addition
...
\gls{microphone}. The camera we have to use is the Avonic CM70 model
(figure \ref{fig:camera}) and the
microphone is the Sennheiser TeamConnect Ceiling 2 (figure \ref{fig:microphone}).
We are not allowed to
reprogram these devices. That is why a single-board computer, the Nvidia Jetson Nano in our case,
...
To recap:
using the camera, \gls{microphone} and single-board computer mentioned above,
we had to find a way to automatically move the camera to point at a speaker after minimal installation effort.
...
a similar problem to the one described in Section \ref{sec:problem-statement}.
Looking at such products helps to get a
better idea of the state-of-the-art solutions in this industry. For every case below, we contrast the
...
Avonic has tried to use a dual-lens design before, but our project is about
using a single camera with a single lens. To make up for the lack of multiple
lenses, we have to use a \gls{microphone}. This way it is still possible to detect
new speakers that are not in the view of the camera. The video processing
techniques used by AVer may be similar to the ones employed by Avonic, but
since the \gls{microphone} was an essential part of our project, the AVer camera
...
which eliminates the need to calculate the speaker's position from the microphone and transfer data between the two devices.
...
Using one camera, Cisco locates the speaker and zooms in on them, while
another camera is watching the entire room to look for the next possible
...
while recording the current one.

The use of two cameras thus makes it easier for Cisco to locate the speaker.
The overall problem is simplified using this approach. We were assigned by Avonic to use only
one camera and one microphone array and try to track speakers as well as companies
like Cisco do with two or more cameras.
\\ \\
As we can see, the most common approach to the problem of pinpointing the speaker is
to use a dual-lens or dual-camera design. On behalf of Avonic, we experimented
with a new approach: one camera and one microphone array. This poses new problems.
Our devised solution is described in \hyperref[ch:solution]{Chapter 4}.

\section{Stakeholders}\label{sec:stakeholders}
This section describes who the stakeholders of our project are and how we have
...
four main types of stakeholders. These people and companies are directly affected by the
result of our project. The types of stakeholders of our project are as follows:

\paragraph{Presenters}\mbox{} \\
People who participate in conferences or giving lectures using MaAT have a high
...
important to the developers that the code is well written and especially well documented.
...
direct contact with presenters or IT staff, so these groups were mainly
represented by the Avonic team.
...
project relies on the availability of both the camera and the microphone array, we would
have been unable to work on it without them. Our contact person from Avonic --
...
the large and expensive ceiling microphone array. We mitigated this by using a
virtual private network (VPN) connection to the office. This way we had a way
to access the \gls{microphone} remotely, although we could not test it very well when
nobody from the group was at the office.

Apart from the camera and the \gls{microphone}, we also needed an Nvidia Jetson Nano
...
and work closely together. All our meetings with Mr. Kahawati
happened on days we could visit the office, so those meetings did not suffer
from our limited time there.

\section{Ethical implications}\label{sec:ethical-implications}
