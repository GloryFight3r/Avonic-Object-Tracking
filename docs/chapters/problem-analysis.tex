...
detect the speaker's location, this is similar to our project. The big
difference is that the Yealink microphone is built into the camera, which
eliminates the need to translate data from the microphone to the camera. Since
we have a seperate microphone at a different position from the camera, we face
an entirely different problem from Yealink.

\paragraph{Cisco}\mbox{} \\
Using one camera, Cisco locates the speaker and zooms in on that speaker, while another camera is watching the entire room to look for the next
