...
\section{Calibration endpoints}

\paragraph{POST /calibration/add\_directions\_to\_speaker}
...
\paragraph{POST /calibration/add\_directions\_to\_mic}
...
\paragraph{POST /calibration/reset}
...
\paragraph{GET /calibration/is\_set}
Endpoint that checks whether the calibration is completed and all the
        needed vectors are calculated

\verb|return:|
\begin{itemize}
  \item \verb|"is_set"|: boolean indicating whether calibration has been completed
  \item HTTP 200: success code
\end{itemize}

\paragraph{GET /calibration/camera}
Endpoint that calculates the relative coordinates of the camera in
    relation to the microphone

\verb|return:|
\begin{itemize}
  \item \verb|"camera_coords"|: a 3D array of floats [x, y, z]
  \item HTTP 200: success code
\end{itemize}

\paragraph{GET /calibration/number-of-calibrated}
...
\section{Info-thread and update endpoints}
