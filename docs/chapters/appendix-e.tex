...
This page includes everything our application has to offer in one neatly organized package
...
\begin{itemize}
    ...
    \item \verb|"vector-z"|: Float representing the z-coordinate of the direction vector.
\end{itemize}
\verb|return:|
\begin{itemize}
    \item{
...
If nobody has spoken in 5 seconds - returns last direction from the microphone.
...
\paragraph{POST /calibration/add-directions-to-speaker} \mbox{} \\
...
\begin{itemize}
    \item{In case of an error:
        \begin{itemize}
            \item HTTP 504: the error code if no direction could be gotten from either the camera
                        or the microphone
        \end{itemize}
        \verb|"message"|: the exact error message
    }
    \item{
        In case of success:
        \begin{itemize}
...
\paragraph{POST /calibration/add-direction-to-mic} \mbox{} \\
...
\begin{itemize}
    \item{In case of an error:
        \begin{itemize}
            \item HTTP 504: the error code if no direction could be gotten from the camera 
        \end{itemize}
        \verb|"message"|: the exact error message
    }
    \item{
...
\paragraph{POST /calibration/reset} \mbox{} \\
...
\paragraph{GET /calibration/is-set} \mbox{} \\
Endpoint that checks whether the calibration is completed and all the
        needed vectors are calculated

\verb|return:|
\begin{itemize}
  \item \verb|"is-set"|: boolean indicating whether calibration has been completed
  \item HTTP 200: success code
\end{itemize}

\paragraph{GET /calibration/camera} \mbox{} \\
Endpoint that calculates the relative coordinates of the camera in
    relation to the microphone

\verb|return:|
\begin{itemize}
  \item \verb|"camera-coords"|: a 3D vector (array) of floats $[x, y, z]$
  \item HTTP 200: success code
\end{itemize}

\paragraph{GET /calibration/number-of-calibrated} \mbox{} \\
Endpoint that returns the number of calibration points

\verb|return:|
\begin{itemize}
  \item \verb|"speaker-points-length"|: the amount of speaker points that have been set $[0, 3]$
  \item HTTP 200: success code
\end{itemize}

\section{Tracking}

\paragraph{GET /track/preset} \mbox{} \\
Endpoint that changes the tracking to use the preset model.

\verb|return:|
\begin{itemize}
    \item{HTTP 200: success, empty message}
    \item \verb|"tracking"|: value representing \verb|PresetModel|
\end{itemize}

\paragraph{GET /track/calibration} \mbox{}\\
Endpoint that changes the tracking to use the audio model

\verb|return:|
\begin{itemize}
    \item{HTTP 200: the model has successfully been set to the \verb|AudioModel|}
    \item \verb|"tracking"|: the enum corresponding to the \verb|AudioModel|
\end{itemize}

\paragraph{GET /track/calibration-no-zoom} \mbox{}\\
Endpoint that changes the tracking to use the audio model without adaptive zooming

\verb|return:|
\begin{itemize}
    \item{HTTP 200: the model has successfully been set to the \verb|AudioModelNoAdaptiveZoom|}
    \item \verb|"tracking"|: the enum corresponding to the \verb|AudioModelNoAdaptiveZoom|
\end{itemize}

\paragraph{GET /track/object} \mbox{}\\
Endpoint that changes the tracking to use the object tracking model

\verb|return:|
\begin{itemize}
    \item{HTTP 200: the model has successfully been set to the \verb|WaitObjectAudioModel|}
    \item \verb|"tracking"|: the enum corresponding to the \verb|WaitObjectAudioModel|
\end{itemize}

\paragraph{GET /track/hybrid} \mbox{}\\
...
\paragraph{GET /update/microphone}\mbox{} \\
...
\paragraph{GET /update/camera}\mbox{} \\
...
Endpoint that sets the settings to the currently active.
