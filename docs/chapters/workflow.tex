...
As mentioned earlier we integrated TDD (test-driven development) in our process but with slight modifications. 
We wrote the tests in parallel with the actual code and not before. 
The reason for this is that most of our features were very dynamic depending on the stage of development and very integrated with each other. 
That meant that we would have to rewrite the tests every time the feature evolves which could be very time-consuming. 
Still, no merge to the main branch was made without having proper tests. 
This apporach once more proved to be a very good practice because we managed to find many bugs and unexpected behavior early in development without breaking the functionality of our project. \paragraph
One of our non-functional requirements was reaching at least 80\% test coverage of the code we write. 
...
During the midterm we were exactly at 80\% coverage with the vision that we would increase that number by the end of the project.
 We indeed met that goal by reaching 92\% total coverage. This high number ensures us that the code written is of high quality and free of many potential bugs. \paragraph
In order to make sure that the tests we wrote were of high quality as well we used mutmut. 
It helped us improve the test suite against possible mutations. 
During this process we managed to locate some bugs throughout the code as well as improving the tests themselves. 
Overall we made sure all our tests cover and capture all aspects of the functionality we have. 
Running this tool, however, posed a difficulty as our system tests take quite a lot of time to run. 
That led to one run of mutmut taking a couple of hours which meant we could not do it that often. 
At the midterm our mutation score was 56\% and we focused on improving that until the end of the project and reached more than 60\%(MORE SPECIFIC?) in the end.\paragraph 
