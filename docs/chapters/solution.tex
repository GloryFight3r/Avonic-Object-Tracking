...
relies on three special hardware devices -- a camera, a microphone and a
single-board computer (SBC). The final product should account for all the inherent
limitations of these devices.

It is important to understand that the microphone allows
us to only retrieve information provided by the Sennheiser which is limited to
the direction toward the speaker and the peak loudness level, since we do not have access to each individual microphone
...
the possible locations of the speaker. We view these options to be infeasible
for us in 10 weeks.
...
To achieve this, the user first sets the microphone's height above the speaker plane.
...
microphone's height above the so-called speaker plane is enough to calculate
...
This can be proven with a geometric proof, or using geometric modeling that we did with GeoGebra\footnote{www.geogebra.org/apps}.
...
search for their location using the microphone information. If they are in the FoV(Field of View)
...
This thread retrieves footage from the Real Time Streaming Protocol (RTSP) stream of the camera using \verb|OpenCV|.
...
\textbf{NOTE:} In the report we have used "active speaker" in multiple places. By that, we understand a person, who is 
