...
A diagram showing the use cases for each of the five different
...
that can be put on the same network as the \gls{microphone} and the camera. This adds
...
relies on three special hardware devices -- a camera, a \gls{microphone} and a
single-board computer (SBC). The final product should account for all the inherent
limitations of these devices.

It is important to understand that the microphone array allows
us to only retrieve information provided by its external API which is limited to
the direction toward the speaker and the peak loudness level.
This means that we do not have access to each individual microphone
within the array.
Since the array itself does not provide the exact origin of the detected sound, as it is
quite a difficult technical challenge to calculate
this\cite{distance-urbana-champaign}, we have no way of obtaining the precise position of the speaker.
...
easy to install and teachers or presenters should be able to calibrate the system
...
impossible without hacking it to get access to each individual
...
This means that we needed to make assumptions about the environment of the room:


\begin{enumerate}
    \item The distance between \gls{speaker-plane} and the ceiling, to which the \gls{microphone} is mounted, is constant.
    \item If the camera, or the \gls{microphone} had been moved, the system must be recalibrated.
...
and the limitations mentioned in \hyperref[sec:solution-challenges]{Section 4.1}.
While assumptions 2 and 3 are in place to guarantee the intended way of
using the system, assumption 1 is formulated to determine the distance from the
\gls{microphone} to the speaker.

From all of our attempts to come up with the solution, we have identified that
the length of the vector from the microphone to the speaker is crucial.
Since the \gls{microphone}'s API\cite{microphone-manual} does not have a way to
retrieve this distance, the assumptions are needed to describe the distance
function in the environment.
According to our assumptions, the \gls{speaker-plane} represents the set of all
locations at which the speaker could be. As was shown by our experiments, small
deviations from the horizontal speaker plane can be neglected due to their
small magnitude. Differences in the height of speakers are the most common
example of this. Such differences in height have even less of an impact when
the speakers are sitting. With this in mind, we decided to calculate the
intersection of the ray from the microphone with the speaker plane to find the
location of the speaker. In the future, it is possible to extend the model to
support different shapes and possibly a full 3D scan of the room to represent
the possible locations of the speaker. We view these options to have been infeasible
for us to implement in 10 weeks.

\subsection{Calibration}\label{subsec:calibration}
During the \gls{calibration} stage, we calculate the distance from the camera to the microphone and therefore obtain the camera's relative coordinates to the microphone.
To achieve this, the user first sets the \gls{microphone-height} above the speaker plane.
Then, a person is sent into the room. They direct the camera at themselves and speak.
The direction obtained from the camera and microphone combined with the
microphone's height above the so-called \gls{speaker-plane} is enough to calculate
the 3D position of the camera relative to the microphone.
...
\begin{figure}[h]
    ...
    \label{fig:cal2d}
\end{figure}
We define $M$ as the position of the \gls{microphone}, $C$ as the camera, and $A$ as the person calibrating the system.
...
Besides the solution using \gls{calibration} described above, we considered three
...
Another solution we came up with, was inspired by the concept of interpolation.
...
This algorithm would only be possible if the camera was located at the same point as the microphone,
or in completely opposite position from the microphone, in relation to the calibration points.
This can be proven with a geometric proof, or using geometric modeling that we did with GeoGebra\footnote{\url{www.geogebra.org/apps}}.
Unfortunately, the camera and \gls{microphone} are not always in the same or opposite
...
have a full 3D environment with all the objects’ arrangements inside. This
...
the system. This does not align with our requirements.
...
smoother and more accurate. The audio discriminates between different people to determine who is
speaking; the video helps to exactly center the camera on this person's face.
...
available for object detection and many models have been trained on these sets. We elaborate on those two models below.
...
the intersection-over-union score. This is the score calculated by intersecting one box with the
...
suppression. This way, RT-DETR manages to achieve a higher speed than YOLO without losing any accuracy\cite{rt-detr}.

Just like YOLO, RT-DETR can be used to track objects in real-time to assist our overall speaker tracking. However,
despite its claim to improved performance, we noticed that in practice the YOLO algorithm was multiple times faster (on one PC, 2 times, to be precise)
than RT-DETR. This is why we ended up using the latest available version of YOLO (YOLOv8) as our object detector.
...
\subsection{Integration of object tracking into our speaker tracker}\label{subsec:integration-of-object-tracking-into-our-speaker-tracker}
The problem of combining two systems of tracking a speaker, both object tracking and speaker tracking, is discussed in
this subsection. A decision has to be made by the system whether and when to listen to the speaker tracker or the object tracker.
There are three strategies we experimented with to deal with this problem.
We review each of them and explain in what kind of situation the strategies work best.
Avonic has asked us to give audio-based speaker tracking priority over object detection using video frames.
Because of this, all strategies we tried use audio to determine the location of the speaker for the most important (i.e.\ biggest) movements.
Since the last two strategies both improve speaker tracking in different ways, the UI (user interface) gives users the option to choose between these methods.
...
movements this speaker makes. This uses both the \gls{microphone}'s ability to detect which person is speaking and the object
detector's ability to precisely (and aesthetically) position a person on a frame.
...
speaker.
For this reason, we do not consider the thresholding strategy to be a good strategy in general.
The two following strategies give a better user experience in all cases.
...
points to the same direction for a variable amount of iterations; one iteration means one time
...
simultaneously with speaker tracking.
This helps to center the speaker on the screen.
...
\begin{figure}[h]
    ...
    \caption{States diagram of the quick change object-audio strategy, which combines object detection with audio information}
    \label{fig:object_strategy}
\end{figure}
\\
The last approach we consider is always relying on microphone coordinates
when there is an \gls{active-speaker}, but furthermore, adjusts the camera based on
camera footage and bounding boxes we get from it. When there is an active speaker, we first
search for their location using the \gls{microphone} information. If they are in the FoV (Field of View)
of the camera, we get the speaker-bounding boxes from the current frame.
After translating the \gls{microphone} direction to a camera direction and finding
the presumed position of the speaker on the screen as a point, we search which bounding box's center is
the closest to that point. After finding it, we direct the camera
to the center of this box.

If the speaker is not
currently visible on the screen, we again translate the \gls{microphone} direction to
...
closest to this new bounding box in the new frame. Assuming that, we get
all bounding boxes from the new frame and choose to track the one
that has its center the closest to the previous bounding box. A diagram that
illustrates the possible states we could be in is the depicted in Figure \ref{fig:object_strategy}.
A visualization of this process can be seen in Figure \ref{fig:obj_detection_image}
...
The Camera API module contains all the functionality related to
communication with the camera. This includes sending commands to point or zoom
...
than HTTP, the higher-level (and therefore slower) alternative.
There are two types of VISCA requests: long and short.
The difference between these two types is the header. Long VISCA requests have a
...
this: they are easier to implement, and we had low traffic, so we could
...
information about the camera such as its position, so every response has to be manually
...
The implementation of the Camera API uses the adapter design pattern,
which is a programming design pattern that allows different interfaces to work together\cite{adapter-pattern}.
The main advantage of this design pattern in our project is that it makes sure our system still works
if the specifics of communication with the camera change; only the adapter itself has to be reimplemented.
This makes it easier to adapt our system to other Avonic cameras.
...
This thread retrieves footage from the Real Time Streaming Protocol (RTSP) stream of the camera using OpenCV\footnote{\url{https://opencv.org/}},
an open source library for computer vision and video-related tasks.
The frames are continually read by the thread so that the latest frame is always available.
This frame can be requested by the web interface to show the stream to the user or by the tracking thread,
which runs YOLO and attempts to find the speaker.


\subsection{Microphone API}\label{subsection:mic}
The Microphone API module contains all the functionality related to
communication with the \gls{microphone}. The most important requests are the one that
gets the direction from the \gls{microphone} towards the detected sound and the one
that gets the peak volume. Like the Camera API, the Microphone API uses an
adapter pattern to facilitate the use of other \glspl{microphone} in the future. \\

\subsection{Tracking}\label{subsection:tracking}
All functionality related to speaker tracking is contained in this
...
so-called model. Every model inherits from the \verb|TrackingModel| class and has
...
Some models rely on data stored in JSON files whose location can be
specified in the settings menu of the UI. We currently have the following five
tracking models:
\begin{enumerate}
\item Preset Model
\item Audio Model
\item Audio Model Without Adaptive Zooming
...
from the \gls{microphone}, this model finds the preset which is the closest to
the given microphone direction according to its cosine similarity. This
...
The \verb|PresetCollection| can be edited from the UI. This is how new preset
locations are added or old ones removed. Added preset locations are reloaded
from a file when the server starts, so there is no need to set them up
...
this simplicity of the installation and its wide-spread usage are the reason why we
...
described in subsection \ref{subsec:calibration} about calibration. The
...
fields from that file are non-existent or corrupted, they are replaced by
default values.
...
Reasons for that inconsistency are the possibility of inaccurate \gls{calibration} or
inaccurate readings provided by the \gls{microphone}.
...
not precise enough to accurately center the speaker. Having identified all
the people in the frame and having created the corresponding bounding boxes
around them, it finds the box that is most likely to be the one
around the \gls{active-speaker}. Then another method calculates the relative direction in
...
currently implemented algorithm, and we have two -- the ``Wait object-audio model''
and the ``Quick change object-audio model'', both described in detail
in \ref{subsec:integration-of-object-tracking-into-our-speaker-tracker}.
...
information, which is used for visualization. The front page of the UI, which
contains most of the UI's functionality, is shown in figure \ref{fig:main-ui}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{main-page-ui}
    \caption{The main page of the web interface}
    \label{fig:main-ui}
\end{figure}

\paragraph{Web UI requirement}\mbox{} \\
The web UI was moved from the “should-have” to the “must-have” section after
...
set, the camera direction towards the \gls{microphone} can be submitted. The reason we
...
accurate. We found that even one calibration point usually gives good accuracy, which is why we allow
...
In the first weeks of working on the project, we have experienced multiple issues
...
microphone APIs and all the threads our application uses -- the update thread,
responsible for tracking; the microphone and camera info-threads, responsible for
sending information updates to the user; and the footage thread for retrieving frames
from the camera.
...
were moved to the \verb|__init__.py| file, so that the integration object can be
passed to every endpoint’s body function. To avoid bloating of the file, we
...
of the development server, Werkzeug. A script was created to easily run the server in
...
\verb|NO_FOOTAGE=true| as an environment variable.
