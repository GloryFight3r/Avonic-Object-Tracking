...
\subsection{Tracking}
All of the functionality related to speaker tracking is separated in this module.
The most important part of it is the \verb|UpdateThread|.
It is a class that extends \verb|Thread| and is responsible for continuous calling of the point method to direct the camera.
The module is structured in a way that every tracking type is maintained in a so-called model.
Every model is an child of the \verb|TrackingModel| class that ensures that they have a \verb|point| method to perform calculations and point the camera.
The picked model is instantiated at the beginning of \verb|run| method that is continuously using the model until the stop signal arrives.
Some of the models rely on data stored in JSON files whose location can be specified in the settings menu in our WebUI.
We currently have the following tracking models:
\begin{enumerate}
\item Preset Model
\item Audio Model
...
The back-end of this app is written in Python using the Flask framework as it is a well-known lightweight framework, that allows for web development.
It offers some nice features - creating and managing socket connections with a client and easily modifiable to support testing of the app.
