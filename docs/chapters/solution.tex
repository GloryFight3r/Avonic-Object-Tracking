...
responsible for continuously the method that directs the camera.
The module is structured in a way that every tracking type is maintained in a
so-called model. Every model is an child of the \verb|TrackingModel| class that
ensures that they have the method to point the camera. The user can pick their
...
The chosen model is instantiated at the beginning of the \verb|run| method that
is continuously using the model until speaker tracking is stopped by the user.
...
from the microphone, the this model finds the preset which is most similar to
...
jittering when following the speaker. The model also saves vectors that were
added to the calibration and can load data from a supplied file. If any of the
...
This model has almost the same functionality as the Audio Model with the only
