...
A flat plane is the most probable case of how the available place for the presenter on stage looks; small inconsistencies in not precisely a flat floor can be neglected due to their small magnitude \textbf{(MAYBE PUT REFERENCE HERE)}.
...
In the future, it is possible to extend the model to support different shapes and possibly a full 3D scan of the room, but we viewed it to be infeasible for us in 10 weeks.
...
The front-end is written in HTML, CSS and JavaScript and includes controls for the camera, the information from the microphone, and a visualization all on one page.
