...
(\hyperref[sec:alternative-solutions]{section 4.3}). Finally, we talk about the object
...
problem described in \hyperref{ch:problem-analysis} has to try to solve. The
...
Unlike the preset model, it can point the camera towards any arbitrary point in the room by performing the calculations described in SECTION ON SOLUTIONS MATH.
...
As explained in section [SECTION NUMBER OF OBJECT TRACKING] we chose to use YOLO for our object detection and classification.
...
The back-end of this app was written in Python using the Flask framework.
