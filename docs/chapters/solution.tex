...
\begin{itemize}
    ...
    \item Manually inputting measures that would describe a coordinate systemß
...
closest to the new bounding box in the new frame. Assuming that, we get
all new bounding boxes from the new frame and choose to track the one
...
so-called model. Every model inherits from the \verb|TrackingModel| and has
...
specified in the settings menu of the UI. We currently have the following
tracking models:
\begin{enumerate}
\item Preset Model
\item Audio Model
...
\verb|PresetCollection| can be edited from the UI. This is how new preset
