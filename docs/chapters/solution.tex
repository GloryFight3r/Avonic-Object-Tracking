...
In this section, we will discuss our application's structure and out motivations behind it.
\subsection{Project modules}
After splitting our project in core domains, we have identified 4 of them:
\begin{itemize}
    \item Camera API - responsible for a simplifying communication with the camera.
    \item Microphone API - responsible for a simplifying communication with the microphone.
    \item Tracking - responsible for all of the tracking related function. For example, calculating direction of the camera, performing communication with camera and the microphone via API.
    \item WebUI - responsible for providing a user-friendly interface for a user.
\end{itemize}
...
It also allows to potentially distribute some of the modules separately, allowing other people develop their software using our APIs.
>>>>>>> Stashed changes
...
the user. Adding more than three points to calibration does not make it more
accurate. One calibration point also gives good accuracy, which is why we allow
