...
def translate_microphone_to_camera_vector(cam_to_mic: np.ndarray,
    ...
    if len(mic_to_speaker.shape) != 1 or (mic_to_speaker.shape[0] != 3):
        raise TypeError("Not a 3D vector")
    assert mic_to_speaker[1] != 0
    cam_to_mic_flatten: np.ndarray = cam_to_mic.flatten()
    mic_to_speaker_flatten: np.ndarray = mic_to_speaker.flatten()
    intersection_point: np.ndarray = -mic_to_speaker_flatten * mic_height \
        / (mic_to_speaker_flatten[1])

    return intersection_point + cam_to_mic_flatten
