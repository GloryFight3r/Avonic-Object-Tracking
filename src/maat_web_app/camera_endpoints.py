...
from maat_web_app.integration import GeneralController, verify_address
from maat_camera_api.camera_adapter import ResponseCode
from maat_camera_api.converter import vector_angle


def responses():
    """ All the possible responses from the camera.
    """
    return {
        ResponseCode.ACK:
...
def navigate_camera(integration: GeneralController):
    ...
    # calculate the camera_speed and camera_rotation
    # we need to make to center the camera on that pixel
    try:
        camera_speed, camera_rotation = \
            integration.hybrid_model.get_movement_to_box(np.array([x, y, x, y]))
    except AssertionError as e:
        return make_response(jsonify({"message": str(e)}), 400)
