...
from avonic_speaker_tracker.object_model import ObjectModel
...
def navigate_camera(integration: GeneralController):
    ...
    try:
        camera_speed, camera_rotation = ObjectModel.get_movement_to_box(np.array([x, y, x, y]), integration.cam_api, integration.footage_thread)
    except AssertionError as e:
        return make_response(jsonify({"message": str(e)}), 400)
